function [MOrange] = f_tensorial_Cluster_Multi_Sub(SourceFile,runs,Comp,MaxIteration,OutPutdir,Method)
% PURPOSE
...
% 
% INPUTS
% MOrange       (vector) the recommended model order range
...
%     sR = New_sR;
    ...
    icassoGraph(sR)
    set(gcf,'visible','off');
    saveas(gca,[ResultFile filesep 'ClustersOfRankone'],'fig');
    RankoneIq{isComp} = iq;
    ConvgNum{isComp} = sum(step<MaxIteration)/runs;
    %% Calculate features
...
for kc = Comp
    mCqCC = nanmean(RankoneIq{kc},2);
    NoH = sum(mCqCC>=0.85);
    NoL = sum(mCqCC<0.5);
    NoM = sum(mCqCC<0.85) - NoL;
    OptMOs(kc) = ConvgNum{kc}*(NoH + 0.5*NoM - 1.5*NoL)/kc;
end

range(1) = find(diff(OptMOs>0.8)==1,1,'first')+1;
range(2) = find(diff(OptMOs>0.8)==-1,1,'first');
MOrange = range(1):range(2);
%%
